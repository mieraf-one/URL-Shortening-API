...
def create_refresh_token(data: dict, db: Session):
    ...
    save_to_db = token_model.RefreshToken(token=encoded_jwt, user_id=to_encode['user_id'])
...
def refresh_token(token: str, db: Session) -> str | HTTPException:
    try:
        ...
        # fetch token from db
        db_token = db.query(token_model.RefreshToken).filter(token_model.RefreshToken.token == token).first()
